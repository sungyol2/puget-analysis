...
class ItineraryCollection:
    def __init__(self, itineraries_df: pandas.DataFrame, region: str):
        ...
        fastest = itineraries_df[["from_id", "to_id", "option", "wait_time", "travel_time"]].fillna(0)
        ...
        fastest_only = pandas.merge(itineraries_df, fastest, on=["from_id", "to_id", "option"])
...
class Itinerary:
    ...
    def compute_fare(self) -> int:
        ...
        while leg.next_leg is not None:
            ...
            df = df[
                ((df.from_route_id == from_leg.route_id) | (df.from_route_id == "__ANY__"))
                & ((df.to_route_id == to_leg.route_id) | (df.to_route_id == "__ANY__"))
            ]
            df = df[
                ((df.from_stop_id == from_leg.end_stop_id) | (df.from_stop_id == "__ANY__"))
                & ((df.to_stop_id == to_leg.start_stop_id) | (df.to_stop_id == "__ANY__"))
            ]
            if df.shape[0] > 0:
                tfr = df.iloc[0]
                ...
                if tfr.transfer_type == "transfer-discount":
                    ...
                    if tfr.new_fare == 0:
                        ...
                        try:
                            self.update_existing_fare(from_leg, discount=int(tfr.fare_value), transfer_decrease=1)
                    ...
                    if tfr.new_fare == 0:
                        try:
                            self.update_existing_fare(from_leg, transfer_decrease=1, cost_increase=tfr.fare_value)
                        except NoExistingFareError:
    ...
    def update_existing_fare(self, leg, transfer_decrease=0, cost_increase=0, discount=0):
        for fare in self._fares:
            if fare.active == True:
...
class ZoneFare(BaseFare):
    def __init__(self, start_time, max_transfers, max_time, feed, route_id, from_zone, to_zone):
        super().__init__(start_time, max_transfers, max_time, feed)
        self.route_id = route_id
